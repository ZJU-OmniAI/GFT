...
class FSDPSFTTrainer:
    ...
    def _compute_loss_and_backward(self, batch, do_backward=True):
        ...
        # batch.keys() -> _StringKeys(dict_keys(['input_ids', 'attention_mask', 'position_ids', 'loss_mask']))
        # batch['input_ids'].shape -> torch.Size([4, 2048])
        # 训练的时候应该是直接补充到2048,<pad>在右边
        ...
        with context, torch.autocast(device_type=self.device_name, dtype=torch.bfloat16):
            if not use_sp:
                ...
                # 输出的logits要往右边移位1格，最后一个token的logits没有可预测的下一个token了
                shift_logits = logits[..., :-1, :].contiguous()
                shift_labels = labels.contiguous()
                
                # Flatten the tokens
                # 将logits和labels展平，shift_logits和shift_labels是一一对应的
                # 也就是说第i个位置的logits是预测第i个位置的labels的
                ...
                # 注意只计算有效token,有效token即:生成答案的那部分token
            ...
            # 计算平均loss,就是 总有效loss / 有效token的数量
    ...
    def training_step(self, batch: TensorDict):
        ...
        # 当前GPU当前step需要训练64条数据,拆分为更小的micro_batch(4),一次性训练
        micro_batches = batch.split(self.config.data.micro_batch_size_per_gpu)
        n_micro_batches = len(micro_batches)
        step_loss = 0
        step_original_loss = 0
        for micro_batch in micro_batches:
            loss, original_loss = self._compute_loss_and_backward(batch=micro_batch) # 这里的loss是4条一起计算出来的
            loss = loss / n_micro_batches # 求出平均每条数据的loss
        ...
        if not torch.isfinite(grad_norm):
            ...
            self.optimizer.step() # 根据计算的梯度执行更新

        log_gpu_memory_usage("After optimizer step", logger=logger)

        self.lr_scheduler.step() # 计算并设置新的学习率，用于下一个训练步骤
    ...
    def fit(self):
        rank = self.device_mesh.get_rank() # 当前运行的进程
        ...
        total_training_steps = len(self.train_dataloader) * self.config.trainer.total_epochs # 1个step对应256条数据
        ...
        for epoch in range(self.config.trainer.total_epochs):
            ...
            for data in tqdm(
                ...
                disable=rank != 0, # 只有主进程(rank=0)才打印输出
            ):
                global_step += 1
...
def run_sft(config): # 入口函数，初始化初始环境
    device_name = get_device_name() # 设备类型:cuda
    local_rank, rank, world_size = initialize_global_process_group() # 初始化进程组

    device_mesh = init_device_mesh(device_type=device_name, mesh_shape=(world_size,), mesh_dim_names=("fsdp",)) # 初始化fsdp使用的设备网络
    dp_size = world_size // config.ulysses_sequence_parallel_size # 数据并行规模
    ...
    local_model_path = copy_to_local(src=config.model.partial_pretrain, verbose=True) # 模型地址
    tokenizer = hf_tokenizer(local_model_path, trust_remote_code=config.model.trust_remote_code)
    train_dataset = create_sft_dataset(config.data.train_files, config.data, tokenizer) # 加载并处理用于SFT的训练数据(重要)

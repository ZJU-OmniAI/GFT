...
from verl.utils.dataset import SFTDataset,GFTDataset
...
class FSDPSFTTrainer:
    ...
    def _compute_loss_and_backward(self, batch, do_backward=True):
        ...
        # 获取question对应的input_ids
        question_length = batch['length'][0].item()
        question_input_ids = copy.deepcopy(input_ids[0][0][:question_length].unsqueeze(0))
        question_attention_mask = copy.deepcopy(attention_mask[0][0][:question_length].unsqueeze(0))
        with torch.no_grad():
        ...
        ####################################################################
        # 这里要实现调用model对现有问题进行推理并把答案转换为input_ids的过程   #
        # 同时针对生成的多条回答，需要重新构建 input_ids loss_mask等         #
        ##################################################################
...
def run_sft(config):
    ...
    train_dataset = create_gft_dataset(config.data.train_files, config.data, tokenizer)
...
@hydra.main(config_path="config", config_name="gft_trainer", version_base=None)
def main(config):
    run_sft(config)


def create_gft_dataset(data_paths, data_config, tokenizer):
    """Create a dataset."""
    # build dataset

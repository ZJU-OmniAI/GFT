...
def setup(args):
    ...
    for data_name in data_list:
        ...
        # 将数据分割给各个进程 500条数据 4个GPU，每个就是125条
        chunk_size = len(examples) // world_size
        processes = []
        
        for rank in range(world_size): # 分进程分别测试数据
...
def main_worker(llm, tokenizer, data_name, args, examples, rank):
    ...
    samples = []
    for example in tqdm(examples, total=len(examples), desc=f"GPU {rank}"): # 处理数据输入
        idx = example["idx"]


        # parse question and answer
        example["question"] = parse_question(example, data_name) # 这个问题是不带提示词模板的啊,是纯问题
        if example["question"] == "":
            continue
        gt_cot, gt_ans = parse_ground_truth(example, data_name) # gt_cot 就是标注的推理过程,gt_ans是最终答案
        example["gt_ans"] = gt_ans
        full_prompt = construct_prompt(example, data_name, args) # 注意:这个函数是嵌套了模板之后对应的函数!
    ...
    ] # 相邻16条数据为一组 一共的输入数据为 500 * 16 = 8000条
    if args.apply_chat_template:
        input_prompts = [
    ...
    for i, sample in enumerate(samples):
        ...
        result = results[i * args.n_sampling : (i + 1) * args.n_sampling] # 拿出第i个问题对应的答案

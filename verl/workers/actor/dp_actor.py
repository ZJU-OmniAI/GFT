...
from verl.trainer.ppo.core_algos import agg_loss, compute_policy_loss, get_policy_loss_fn, kl_penalty, compute_policy_loss_raft, compute_policy_loss_raft_scale, compute_sft_loss,compute_gft_loss
...
class DataParallelPPOActor(BasePPOActor):
    ...
    def _forward_micro_batch(
        ...
        Returns:
            ...
            probs: # (bs, response_len) <<<<<<<<<< [NEW RETURN VALUE] 从原始logits计算的概率，无精度损失
        """
        ...
        with torch.autocast(device_type=self.device_name, dtype=torch.bfloat16):
            ...
            if self.use_remove_padding:
                ...
                if self.use_fused_kernels:
                    ...
                    logits_rmpad_original = logits_rmpad.clone()  # 保存原始logits用于计算概率，未经过temperature调整
                    ...
                    # ----------------Compute Prob--------------------
                    # 从原始logits计算概率（保持最高精度，避免exp(log_prob)的精度损失）
                    probs_rmpad = torch.softmax(logits_rmpad_original, dim=-1)  # (total_nnz, vocab_size)
                    # 提取对应response token的概率
                    probs_rmpad = probs_rmpad.gather(-1, input_ids_rmpad_rolled.unsqueeze(-1)).squeeze(-1)  # (total_nnz,)
                    # -------------------------------------------------
                ...
                if self.use_ulysses_sp:
                    ...
                    # -----gather and unpad probs for the ulysses sp --------
                    probs_rmpad = gather_outpus_and_unpad(
                        probs_rmpad,
                        gather_dim=0,
                        unpad_dim=0,
                        padding_size=pad_size,
                ...
                # ------------pad back probs to (bsz, seqlen) --------
                full_probs = pad_input(
                    hidden_states=probs_rmpad.unsqueeze(-1),
                    indices=indices,
                    batch=batch_size,
                    seqlen=seqlen,
                )
                ...
                # -------------extract response part of probs -----------
                probs_rmpad = full_probs.squeeze(-1)[:, -response_length - 1 : -1]  # (bsz, response_length)
                # -------------------------------------------------------
                
            else:  # not using rmpad and no ulysses sp (No)
                extra_args = {}
                if self.use_fused_kernels:
                    extra_args["temperature"] = temperature
                    extra_args["return_dict"] = True
            ...
            # ---------- [MODIFIED RETURN] -----------
            return entropy, log_probs, probs_rmpad
            # ----------------------------------------
    ...
    def compute_log_prob(self, data: DataProto, calculate_entropy=False) -> torch.Tensor:
        ...
        for micro_batch in micro_batches:
            ...
            with torch.no_grad():
                entropy, log_probs, probs = self._forward_micro_batch(
        ...
        probs = torch.concat(probs_lst, dim=0)
        entropys = None
        if calculate_entropy:
            entropys = torch.concat(entropy_lst, dim=0)
        if use_dynamic_bsz: # (No)
            indices = list(itertools.chain.from_iterable(indices))
            assert len(indices) == log_probs.size(0), f"{len(indices)} vs. {log_probs.size()}"
            revert_indices = torch.tensor(get_reverse_idx(indices), dtype=torch.long)
            log_probs = log_probs[revert_indices]
            probs = probs[revert_indices]
            if calculate_entropy:
                entropys = entropys[revert_indices]

        return log_probs, entropys, probs

    @GPUMemoryLogger(role="dp actor", logger=logger)
    def update_policy(self, data: DataProto):
        # make sure we are in training mode
        # breakpoint()
        self.actor_module.train() # 设置为训练模式

        temperature = data.meta_info["temperature"]  # temperature must be in the data.meta_info to avoid silent error
        multi_turn = data.meta_info.get("multi_turn", False)

        if "old_log_probs" not in data.batch.data.keys():
        ...
        if has_multi_modal_inputs:
            ...
            dataloader = batch.split(self.config.ppo_mini_batch_size) # 把准备好的数据包括计算的优势,ref_log_prob等准备好为一个dataloader准备更新
        ...
        for epoch in range(self.config.ppo_epochs):
            for batch_idx, data in enumerate(dataloader):
                ...
                for data in micro_batches:
                    ...
                    responses = data["responses"] # shape(16, 1024)
                    response_length = responses.size(1) # 1024
                    attention_mask = data["attention_mask"] # shape(16,2048) 这是整个输入加输出的attention mask,为1的地方代表有输入或者输出
                    if multi_turn:
                        response_mask = data["loss_mask"][:, -response_length:]
                    else:
                        response_mask = attention_mask[:, -response_length:] # 这是输出的response_mask

                    old_log_prob = data["old_log_probs"] if "old_log_probs" in data else None
                    ...
                    entropy, log_prob, prob = self._forward_micro_batch(
                    ...
                    if loss_mode == "vanilla":
                        ...
                        pg_loss, pg_clipfrac, ppo_kl, pg_clipfrac_lower = compute_sft_loss(
                            log_prob=log_prob,
                            prob=prob,
                            advantages=advantages,
                            response_mask=response_mask,
                        ...
                        pg_loss, pg_clipfrac, ppo_kl, pg_clipfrac_lower = compute_gft_loss(
                            log_prob=log_prob,
                            prob=prob,
                            advantages=advantages,
                            response_mask=response_mask,
                    ...
                    if self.config.use_kl_loss: # 是否用KL Loss
